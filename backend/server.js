...
function userPayload(user) {
  return {
    ...
    user_metadata: { name: user.full_name, full_name: user.full_name },
  };
}
...
function authOptional(req, _res, next) {
  ...
  if (token) {
    try {
      ...
      /* Treat expired guest tokens as anonymous. */
    }
  }
  next();
}
...
function mlServiceRequired(req, res, next) {
  if (!process.env.ML_SERVICE_SECRET) {
    return res
      .status(503)
      .json({ error: "ML service integration is not configured" });
  }
...
function oauthState(provider) {
  return jwt.sign({ provider, nonce: crypto.randomUUID() }, jwtSecret, {
    expiresIn: "10m",
  });
}
...
function signedIntegrationState(userId) {
  return jwt.sign({ sub: userId, integration: "fitbit" }, jwtSecret, {
    expiresIn: "10m",
  });
}
...
async function fitbitToken(token, userId) {
  ...
  await pool.query(
    `UPDATE wellness_devices SET access_token = $1, refresh_token = $2, token_expires_at = NOW() + ($3 || ' seconds')::interval WHERE user_id = $4 AND platform = 'fitbit'`,
    [
...
async function updateResource(config, userId, id, item) {
  ...
  if (!assignments.length) {
    const existing = await pool.query(
      `SELECT * FROM ${config.table} WHERE user_id = $1 AND id = $2`,
      [userId, id],
    );
...
async function deleteResource(config, userId, id) {
  if (id)
    return pool.query(
      `DELETE FROM ${config.table} WHERE user_id = $1 AND id = $2`,
      [userId, id],
    );
...
app.post("/api/auth/login", authLimiter, async (req, res) => {
  try {
    ...
    res.json({
      ...
      session: { access_token: tokenFor(user) },
    });
  } catch (error) {
    publicError(res, error);
  }
...
app.patch("/api/profile", authRequired, async (req, res) => {
  ...
  const result = await pool.query(
    `UPDATE profiles SET ${assignments.join(", ")} WHERE user_id = $1 RETURNING *`,
    values,
  );
  res.json({ data: result.rows[0] || null });
});
...
app.get("/api/music/recommendations", authRequired, async (req, res) => {
  const latest = await pool.query(
    "SELECT final_energy_level FROM wellness_scores WHERE user_id = $1 ORDER BY computed_at DESC LIMIT 1",
    [req.auth.sub],
  );
...
`;
    ...
    if (!message) {
      return res
        .status(502)
        .json({ error: "SAATHI returned an empty response" });
    }
...
app.get("/api/auth/oauth/google/callback", async (req, res) => {
  try {
    ...
    res.redirect(
      `${frontendOrigin}/auth?oauth_error=${encodeURIComponent(error.message || "Google login failed")}`,
    );
  }
});
...
app.get("/api/auth/oauth/apple/start", (_req, res) => {
  if (!oauthConfigured("apple"))
    return res.status(503).json({ error: "Apple OAuth is not configured" });
  const params = new URLSearchParams({
...
app.post("/api/auth/oauth/apple/callback", async (req, res) => {
  try {
    ...
    res.redirect(
      `${frontendOrigin}/auth?oauth_error=${encodeURIComponent(error.message || "Apple login failed")}`,
    );
  }
});
...
app.get("/api/integrations/fitbit/start", authRequired, async (req, res) => {
  if (!fitbitConfigured())
    return res
      .status(503)
      .json({ error: "Fitbit integration is not configured" });
  const params = new URLSearchParams({
    response_type: "code",
...
app.get("/api/integrations/fitbit/callback", async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO wellness_devices (user_id, platform, device_name, access_token, refresh_token, token_expires_at) VALUES ($1, 'fitbit', 'Fitbit', $2, $3, NOW() + ($4 || ' seconds')::interval)`,
      [
        state.sub,
        token.access_token,
        token.refresh_token,
...
app.get("/api/integrations/fitbit/metrics", authRequired, async (req, res) => {
  try {
    ...
    if (!device.rows[0])
      return res.status(404).json({ error: "Fitbit is not connected" });
...
app.post("/api/community/rooms", authRequired, async (req, res) => {
  ...
  if (cleanName.length < 3)
    return res
      .status(400)
      .json({ error: "Community name must be at least 3 characters" });
...
app.post("/api/anonymous-posts", authRequired, async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Photo must be an image smaller than 10 MB" });
  }
...
app.post(
  ...
  `,
    ...
    const post = await pool.query(
      "SELECT id FROM anonymous_posts WHERE id = $1 AND moderation_status = 'approved'",
      [req.params.postId],
    );
...
//
// Updating a mood is also a meaningful wellness activity.
// Every actual mood/energy change creates a NEW activity_history row.
//
// Example:
// Happy 80 → Calm 65  = activity_history row
// Calm 65  → Sad 30   = activity_history row
// Happy 80 → Happy 80 = no new activity
//
// The moods table keeps the latest/current mood.
// activity_history keeps the complete history of meaningful changes.
// =================================================================
app.patch("/api/data/moods", authRequired, async (req, res) => {
  ...
  try {
    ...
    // Get the current mood before updating it.
    ...
    // Only allow the actual mood columns to be updated.
    const assignments = [];
    ...
    // -------------------------------------------------------------
    // Detect whether this is a meaningful mood change.
    // -------------------------------------------------------------

    const moodChanged =
      String(existing.mood ?? "") !== String(updated.mood ?? "");

    const oldEnergy =
      existing.energy_level === null || existing.energy_level === undefined
        ? null
        : Number(existing.energy_level);
    ...
    const energyChanged = oldEnergy !== newEnergy;

    // Only create activity_history when mood or energy actually changed.
    if (moodChanged || energyChanged) {
      const activity = await recordActivity(client, {
        ...
        metadata: {
          mood_id: updated.id,
          previous_mood: existing.mood,
          previous_energy_level: existing.energy_level,
          mood_changed: moodChanged,
          energy_changed: energyChanged,
        },
      });

      console.log(`[ACTIVITY] mood change recorded: ${activity.id}`);
    }
...
app.patch("/api/data/meditation_sessions", authRequired, async (req, res) => {
  const id = req.query.id;
  if (!id)
    return res.status(400).json({ error: "An id query parameter is required" });
...
app.patch("/api/data/activity_history", authRequired, async (req, res) => {
  try {
    ...
    if (!id)
      return res
        .status(400)
        .json({ error: "An id query parameter is required" });
    ...
    if (
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "energy_level must be a number between 1 and 100" });
      }
    }
...
app.post("/api/data/:resource", authRequired, async (req, res) => {
  try {
    ...
    for (const item of items)
      data.push(await insertResource(config, req.auth.sub, item));
...
io.on("connection", (socket) => {
  ...
  socket.on("typing:start", (roomId) =>
    socket.to(roomId).emit("typing:update", { roomId, userId, typing: true }),
  );
  socket.on("typing:stop", (roomId) =>
    socket.to(roomId).emit("typing:update", { roomId, userId, typing: false }),
  );
  ...
  socket.on("message:react", async ({ roomId, messageId, reactions }) => {
    ...
    if (selected) {
      ...
      await pool.query(
        "INSERT INTO community_message_reactions (message_id, user_id, user_name, emoji) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING",
