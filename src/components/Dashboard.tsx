...
export default function Dashboard(): JSX.Element {
  ...
  const [notification, setNotification] = useState<WellnessNotification | null>(
    null,
  );
  ...
  const generatePrediction = async () => {
    try {
      ...
      if (!token) {
        setPredictionError("Please log in again.");
      ...
      if (!response.ok) {
        throw new Error(data?.error || "Unable to generate wellness insight");
      }
  ...
  useEffect(() => {
    const loadLatestNotification = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE}/api/notifications/latest`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const getPredictionLevel = (score: number) => {
    if (score <= 30) {
      return {
        label: "Gentle reset recommended",
        icon: Wind,
      };
    }
    ...
    if (score <= 55) {
      return {
        label: "A little support could help",
        icon: Activity,
      };
    }
    ...
    if (score <= 75) {
      return {
        label: "Your pattern looks balanced",
        icon: HeartPulse,
      };
    }

    return {
      label: "Keep your positive routine going",
      icon: Sparkles,
    };
  };

  const predictionLevel = prediction
    ? getPredictionLevel(prediction.predicted_energy_level)
    : null;
  ...
  const startRecommendedActivity = () => {
    ...
    const activity = prediction.recommendation_activity.toLowerCase();
    ...
    if (activity.includes("mindful")) {
      navigate("/meditation");
      return;
    }

    if (activity.includes("reset")) {
      navigate("/meditation");
      return;
    }
            ...
            {!prediction ? (
              <div className="mt-7 rounded-[26px] border border-dashed border-primary/15 bg-primary/[0.025] p-7 text-center">
                ...
                <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
                  Saathi can turn your recent wellness activity into a simple
                  supportive insight.
                </p>
                ...
                <div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-violet-50 to-fuchsia-50 p-6 text-center">
                  ...
                  {predictionLevel && (
                    <div className="relative mt-4 flex items-center justify-center gap-2">
                      {(() => {
                        const PredictionIcon = predictionLevel.icon;

                        return (
                          <PredictionIcon className="h-4 w-4 text-violet-500" />
                        );
                      })()}
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-3">
                    ...
                    >
                      ...
                      {predictionLoading ? "Refreshing..." : "Refresh insight"}
                    </button>
                  </div>
