...
export default function ChatRoom({ room }: Props) {
  ...
  const [supportTarget, setSupportTarget] = useState<BubbleMessage | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    socket.on("room:presence", ({ count }: { count: number }) =>
...
const saveMessage = async (message: ChatMessage) => {
  ...
  const handleEmoji = (message: BubbleMessage, emoji: string) => {
    setMessages((prev) =>
      prev.map((item) => {
        ...
        if (existingIndex === -1) {
          ...
          if (alreadyReacted) {
            ...
            existingReaction.users = [...existingReaction.users, "me"];
          }
        }
        ...
        void supabase
          ...
          .update({ reactions })
          .eq("id", item.id);
        getCommunitySocket().emit("message:react", {
          roomId: room.id,
          messageId: item.id,
          reactions,
        });
        ...
        {messages.length === 0 ? (
          ...
          <div className="space-y-5">
            <MessageList
              ...
              onEmoji={(message, emoji) => handleEmoji(message, emoji)}
            />
